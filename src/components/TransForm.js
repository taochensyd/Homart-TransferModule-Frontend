...
const TransForm = ({ username }) => {
    ...
    const binLocations = (selectedFromWarehouse) => {
    ...
    const changeToBinCode = (selectedToWarehouse, binList) => {
        setToBin([])

        console.log('binList:', binList);
        console.log('selectedToWarehouse:', selectedToWarehouse);

        const filteredBinList = binList.filter(item => item.Warehouse === selectedToWarehouse);
    ...
    function submitTransfer(itemCode, enteredQuantity, selectedFromWarehouse, selectedFromBin, selectedToWarehouse, selectedToBin, remarks, journalMemo) {

        // Submit the data
        const body = {}; // Placeholder for the body data
    ...
    const updateMaxQuantity = (selectedFromWarehouse, selectedFromBin, batchInBin) => {


        if (selectedFromWarehouse === "" && selectedFromBin === "" && batchInBin && batchInBin.length > 0) {
            setMaxQuantity(batchInBin.reduce((acc, item) => acc + item.OnHandQty, 0));
            console.log(`updateMaxQuantity selectedFromWarehouse and selectedFromBin ${selectedFromWarehouse, selectedFromBin}`)
            return;
        }
        ...
        if (selectedFromWarehouse !== "" && batchInBin && batchInBin.length > 0) {
            ...
            console.log(`updateMaxQuantity selectedFromWarehouse ${selectedFromWarehouse}`)
    ...
    useEffect(() => {
        setSelectedToBin("");
        changeToBinCode(selectedToWarehouse, binList)
    }, [selectedToWarehouse]);
    ...
    useEffect(() => {
        ...
        if (selectedFromWarehouse !== "") {
            setFromBin(fromBinList.filter(bin => bin.slice(0, 3) === selectedFromWarehouse));
        }
        ...
        if (selectedFromWarehouse !== "") {
            ...
            console.log(`useEffect From Selected Warehouse: ${selectedFromWarehouse}`)
        }
    ...
    return (
        <div className="flex-container">
            ...
            <div className="flex-item">
                ...
                />
                <button className="ignore-button" onClick={() => setBatchNumber(inputBatchNumber)}>
                    Search
                </button>
            </div>
            ...
            <div className="flex-item">
                <label>Remarks:</label>
                <textarea onChange={e => setEnteredRemark(e.target.value || '')}></textarea>
            </div>
            ...
            <div className="flex-item">
                <button className="ignore-button" onClick={() => clearAll()}>Clear All</button>
                <button className="invisible-button">Add</button>
                <button className="ignore-button" onClick={() => setShowSummary(true)}>Transfer</button>
            </div>
            ...
            <div>
                {showSummary && (
                    <>
                        ...
                        <button className="ignore-button" onClick={cancelTransfer}>Cancel</button>
                        <button className="ignore-button" onClick={submitTransfer}>Submit</button>
                    </>
                )}
            </div>
        </div >
